...
| Выгрузить метрики Comet ML и построить воспроизводимые графики | скилл `koi-comet-report` — команда `inspect` находит доступные метрики, а `build` сохраняет исходные точки в CSV, сводку и манифест в JSON, а также графики в PNG |
